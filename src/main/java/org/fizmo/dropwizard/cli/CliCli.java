...
public class CliCli {
    ...
    private final SortedMap<String, CliCommand> commands;
     ...
     */
    public CliCli(JarLocation location, CliBootstrap<?> bootstrap, OutputStream stdOut, OutputStream stdErr) {
        ...
        for (CliCommand command : bootstrap.getCommands()) {
            addCommand(command);
        }
    }
        ...
        try {
            if (isFlag(HELP, arguments)) {
                ...
                if (namespace.get("is-help") == null) {
                    final CliCommand command = commands.get(namespace.getString(COMMAND_NAME_ATTR));
